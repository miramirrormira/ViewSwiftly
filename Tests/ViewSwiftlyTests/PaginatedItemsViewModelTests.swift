...
final class PaginatedItemsViewModelTests: XCTestCase {
    ...
    func test_convenient_init_for_url_requestables_onFetchItems_should_be_triggered() async throws {
        ...
        let transform: (Page) -> [Item] = { $0.items }
        let sut = PaginatedItemsViewModel(networkConfiguration: networkConfiguration, endpoint: endpoint, paginationQueryStrategy: paginationQueryStrategy, fetchItemsStrategy: .init(fetchItemsStrategySpy), transform: transform)
