...
final class AppendItemsTests: XCTestCase {
    ...
    func test_merge_with_empty_newItems_array() async throws {
        let sut = AppendItems<Item>()
        let vmStub = PaginatedItemsViewModel<Item>.init(requestable: AnyRequestable<[Item]>(RequestableDummy()), mergeItemsStrategy: .init(sut))
    ...
    func test_merge_with_non_empty_newItems_array() async throws {
        ...
        let sut = AppendItems<Item>()
        let vmStub = PaginatedItemsViewModel<Item>(requestable: AnyRequestable<[Item]>(RequestableDummy()), mergeItemsStrategy: .init(sut))
        XCTAssertEqual(vmStub.state.items.count, 0)
