...
final class AppendItemsTests: XCTestCase {
    ...
    func test_merge_with_empty_newItems_array() async throws {
        let sut = AppendItems<Item, Item>()
        let vmStub = PaginatedItemsViewModel<Item, Item>.init(requestable: AnyRequestable<[Item]>(RequestableDummy()), mergeItemsStrategy: .init(sut))
    ...
    func test_merge_with_non_empty_newItems_array() async throws {
        ...
        let sut = AppendItems<Item, Item>()
        let vmStub = PaginatedItemsViewModel<Item, Item>(requestable: AnyRequestable<[Item]>(RequestableDummy()), mergeItemsStrategy: .init(sut))
        XCTAssertEqual(vmStub.state.items.count, 0)
